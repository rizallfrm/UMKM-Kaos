...
import { GET_FEATURED_PRODUCTS, GET_PRODUCT_REVIEWS } from "../graphql/queries";
...
export default function HomePage({ productId }) {
  const { dataRev, loadingRev, errorRev } = useQuery(GET_PRODUCT_REVIEWS, {
    variables: { productId },
  });

  if (loadingRev) return <p>Loading...</p>;
  if (errorRev) return <p>Error loading reviews</p>;

  const reviews = dataRev?.getProductReviews || [];
